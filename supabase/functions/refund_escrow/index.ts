...
serve(async (req) => {
  ...
  try {
    const { orderId, reason, partialAmount } = await req.json();
    ...
    // 2. Récupérer l'ID interne de l'utilisateur connecté (celui qui rembourse)
